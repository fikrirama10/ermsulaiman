...
@section('css')
    <style>
        ...
        .info-label { color: #7e8299; font-weight: 600; font-size: 0.9rem; }
        .info-val { font-weight: 500; color: #181c32; text-align: right; }
            ...
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                    <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                        <h1 class="page-heading d-flex flex-column justify-content-center text-dark fw-bold fs-3 m-0">Registrasi Kunjungan</h1>
                        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                            <li class="breadcrumb-item text-muted"><a href="#" class="text-muted text-hover-primary">Pasien</a></li>
                        ...
                        <div class="card card-flush shadow-sm mb-5">
                            <div class="patient-card-header">
                                ...
                                <div class="d-flex gap-2">
                                    <button class="btn btn-sm btn-white btn-color-gray-700 btn-active-opacity w-100" data-bs-toggle="modal" data-bs-target="#general_concent">
                            ...
                            <div class="card-body pt-4">
                                ...
                                <div id='alert-finger' class="alert alert-{{ $cek_finger_print['response']['kode'] == 0 ? 'danger' : 'success' }} d-flex align-items-center p-3 mb-4 rounded border-dashed border-{{ $cek_finger_print['response']['kode'] == 0 ? 'danger' : 'success' }} d-none">
                                    <i class="ki-outline ki-fingerprint-scanning fs-2hx text-{{ $cek_finger_print['response']['kode'] == 0 ? 'danger' : 'success' }} me-3"></i>
                                    <div class="d-flex flex-column">
                                        <h5 class="mb-1 text-{{ $cek_finger_print['response']['kode'] == 0 ? 'danger' : 'success' }} fs-7">Validasi Finger Print</h5>
                                        <span class="fs-8 text-muted">{{ $cek_finger_print['response']['status'] }}</span>
                                    </div>
                                </div>
                                @endif
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="info-row"><span class="info-label">NIK</span><span class="info-val">{{ $pasien->nik }}</span></div>
                                    <div class="info-row"><span class="info-label">BPJS</span><span class="info-val">{{ $pasien->no_bpjs }}</span></div>
                                    <div class="info-row"><span class="info-label">No HP</span><span class="info-val">{{ $pasien->nohp }}</span></div>
                                    <div class="info-row"><span class="info-label">Lahir</span><span class="info-val">{{ date('d M Y', strtotime($pasien->tgllahir)) }}</span></div>
                                    <div class="info-row"><span class="info-label">Usia</span><span class="info-val">{{ $pasien->usia_tahun }} th {{ $pasien->usia_bulan }} bln</span></div>
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="info-row"><span class="info-label">Kesatuan</span><span class="info-val">{{ $pasien->kesatuan }}</span></div>
                                    <div class="info-row"><span class="info-label">Pangkat</span><span class="info-val">{{ $pasien->pangkat }}</span></div>
                                    <div class="info-row"><span class="info-label">NRP</span><span class="info-val">{{ $pasien->nrp }}</span></div>
                                </div>
                            ...
                            <div class="card shadow-sm mb-5" id="tambah_kunjungan">
                                ...
                                <div class="card-body">
                                        ...
                                        <div class="col-md-6">
                                            <div class="fv-row mb-4">
                                                ...
                                                <div class="position-relative d-flex align-items-center">
                                                    ...
                                                    <input class="form-control form-control-solid ps-12 flatpickr-input" name="tglmasuk" placeholder="Pilih Tanggal" id="kt_datepicker_1" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                             <div class="fv-row mb-4">
                                                ...
                                                <select name="penanggung" class="form-select form-select-solid" id="penanggung">
                                                    <option value="">-- Pilih Penanggung --</option>
                                                    <option value="2">BPJS Kesehatan</option>
                                                    <option value="1">UMUM / Pribadi</option>
                                                </select>
                                                <div class="form-check form-switch form-check-custom form-check-solid mt-3">
                                                    <input class="form-check-input" type="checkbox" value="1" id="buat_sep" name="buat_sep" />
                                                    <label class="form-check-label fw-semibold text-gray-700" for="buat_sep">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="fv-row mb-4">
                                                ...
                                                <select name="jenis_rawat" class="form-select form-select-solid" onchange="updateOptions()" id="jenis_rawat">
                                        ...
                                        <div class="col-md-6">
                                             <div class="fv-row mb-4">
                                                ...
                                                <select name="idpoli" class="form-select form-select-solid" id="poli">
                                    ...
                                    <div class="fv-row mb-4">
                                        ...
                                        <div class="input-group">
                                            <span class="input-group-text border-0 bg-light"><i class="ki-outline ki-stethoscope fs-2"></i></span>
                                            <input type="text" name='dokter' id='dokter' readonly class="form-control form-control-solid" placeholder="Pilih Dokter..." required style="background-color: #f5f8fa; cursor: pointer;">
                                        ...
                                        <div class="form-text text-muted">Pastikan tanggal dan poli sudah dipilih untuk mencari jadwal dokter</div>
                                    </div>
                                    ...
                                    <div class="d-flex align-items-center bg-light-warning rounded p-3 mb-4">
                                        ...
                                        <div class="form-check form-check-custom form-check-solid">
                                            <input class="form-check-input" type="checkbox" name="anggota" value="1" id="flexCheckDefault" />
                                ...
                                <div class="card-footer d-flex justify-content-end py-6 px-9">
                                    <button type="button" id="button-simpan" class="btn btn-lg btn-primary">
                                        <span class="indicator-label"><i class="ki-outline ki-save-2 fs-2 me-1"></i> Simpan Pendaftaran</span>
                            ...
                            <div id="card_buat_sep" class="card shadow-sm border-dashed border-primary d-none mt-5">
                                <div class="card-header bg-light-primary">
                                    ...
                                    <div class="card-toolbar">
                                        <button type="button" id='btn-sep-manual' class="btn btn-sm btn-outline btn-outline-dashed btn-outline-warning btn-active-light-warning">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" id='kt_block_ui_4_target_consent'>
                ...
                <div class="modal-body scroll-y mh-400px">
                    ...
                    <form action="{{ route('pasien.post-form-concent') }}" id="form-consent" method="POST">
                        ...
                        <div class="fv-row bg-light p-4 rounded">
                            ...
                            <input type="text" class="form-control" name="penanggung_jawab" placeholder="Masukkan nama penanggung jawab...">
                ...
                <div class="modal-footer">
                    ...
                    <div class="alert alert-dismissible bg-light-success border border-success border-3 d-flex flex-column flex-sm-row w-100 p-5 mb-0">
                         <i class="ki-outline ki-check-circle fs-2hx text-success me-4"><span class="path1"></span><span class="path2"></span></i>
                    ...
                    @else
                    <button type="button" class="btn btn-primary w-100" id="btn-setuju-consent">Saya Setuju & Simpan</button>
                    @endif
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const UIHelper = {
            showLoading: (message = 'Processing...') => {
                $.blockUI({ message: `<div class="d-flex align-items-center justify-content-center py-3"><div class="spinner-border text-white me-3"></div> <span class="text-white fs-6 fw-bold">${message}</span></div>`, css: { border: 'none', padding: '15px', backgroundColor: '#000', '-webkit-border-radius': '10px', '-moz-border-radius': '10px', opacity: .7, color: '#fff' } });
            },
            showElementLoading: (selector, message = 'Processing...') => {
                $(selector).block({ message: `<div class="d-flex align-items-center justify-content-center"><div class="spinner-border text-primary me-2"></div> <span>${message}</span></div>`, css: { border: 'none', padding: '10px', backgroundColor: 'rgba(255,255,255,0.8)', opacity: 1, color: '#333' } });
            },
        ...
        const AlertHelper = {
            confirmSave: (title = 'Simpan Data', text = 'Pastikan data yang diinput sudah benar.') => {
                return Swal.fire({ title, text, icon: 'question', showCancelButton: true, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Ya, Simpan', cancelButtonText: 'Batal' });
            },
            getConsentSuccessHTML: () => `<div class="alert alert-dismissible bg-light-success border border-success border-3 d-flex flex-column flex-sm-row w-100 p-5 mb-0"><i class="ki-outline ki-check-circle fs-2hx text-success me-4"></i><div class="d-flex flex-column pe-0 pe-sm-10"><h5 class="mb-1 text-success">Sudah Disetujui</h5><span>General Consent sudah ditandatangani.</span></div></div>`
        };

        const AjaxHelper = {
            request: (options) => {
                const defaultOptions = { method: 'GET', loadingMessage: 'Loading ...', showLoading: true };
                const config = { ...defaultOptions, ...options };
                return $.ajax({
                    type: config.method, url: config.url, data: config.data || {},
                    beforeSend: () => { if (config.showLoading) UIHelper.showLoading(config.loadingMessage); if (config.beforeSend) config.beforeSend(); },
                    success: (response) => { if (config.showLoading) UIHelper.hideLoading(); if (config.success) config.success(response); },
                    error: (xhr, status, error) => { if (config.showLoading) UIHelper.hideLoading(); const errorMessage = xhr.responseJSON?.message || error; toastr.error(errorMessage); if (config.error) config.error(xhr, status, error); }
                });
            }
        };
        ...
        const FormValidationSetup = {
            createValidator: (formElement, fields) => {
                return FormValidation.formValidation(formElement, { fields, plugins: { trigger: new FormValidation.plugins.Trigger(), bootstrap: new FormValidation.plugins.Bootstrap5({ rowSelector: '.fv-row', eleInvalidClass: '', eleValidClass: '' }) } });
            },
            setupConsentValidation: () => {
                const form = document.getElementById('form-consent');
                return FormValidationSetup.createValidator(form, { 'penanggung_jawab': { validators: { notEmpty: { message: 'Wajib diisi' } } } });
            },
            setupMainFormValidation: () => {
                ...
                const fields = {};
                requiredFields.forEach(field => { fields[field] = { validators: { notEmpty: { message: 'Wajib diisi' } } }; });
        ...
        const SEPWizard = {
            showTujuanKunjungan: () => Swal.fire({ title: 'Tujuan Kunjungan', html: `<select id="firstSelect" class="form-select"><option value="0" disabled selected>-- Pilih Tujuan --</option><option value="1">Prosedur</option><option value="2">Konsul Dokter</option></select>`, preConfirm: () => { const val = $('#firstSelect').val(); if(!val) Swal.showValidationMessage('Pilih tujuan'); return val; } }),
            showAlasanTidakSelesai: () => Swal.fire({ title: 'Alasan Belum Selesai', html: `<select id="secondSelect" class="form-select"><option value="" disabled selected>-- Pilih Alasan --</option><option value="1">Poli spesialis tidak tersedia</option><option value="2">Jam Poli telah berakhir</option><option value="3">Dokter Spesialis tidak praktek</option><option value="4">Atas Instruksi RS</option><option value="5">Tujuan Kontrol</option></select>`, showCancelButton: true, preConfirm: () => { const val = $('#secondSelect').val(); if(!val) Swal.showValidationMessage('Pilih alasan'); return val; } }),
            showProsedurBerkelanjutan: () => Swal.fire({ title: 'Prosedur Berkelanjutan', html: `<select id="prosedur" class="form-control"><option value="">-- Pilih Prosedur --</option><option value="1">Radioterapi</option><option value="2">Kemoterapi</option><option value="3">Rehabilitasi Medik</option><option value="4">Rehabilitasi Psikososial</option><option value="5">Transfusi Darah</option><option value="6">Pelayanan Gigi</option><option value="12">HEMODIALISA</option></select>`, showCancelButton: true, preConfirm: () => { const val = $('#prosedur').val(); if(!val) Swal.showValidationMessage('Pilih prosedur'); return val; } }),
            showProsedurTidakBerkelanjutan: () => Swal.fire({ title: 'Prosedur Tidak Berkelanjutan', html: `<select id="nonprosedur" class="form-control"><option value="">-- Pilih Prosedur --</option><option value="7">Laboratorium</option><option value="8">USG</option><option value="9">Farmasi</option><option value="10">Lain-Lain</option><option value="11">MRI</option></select>`, showCancelButton: true, preConfirm: () => { const val = $('#nonprosedur').val(); if(!val) Swal.showValidationMessage('Pilih prosedur'); return val; } }),
            showBerkelanjutanChoice: (onYes, onNo) => Swal.fire({ title: 'Prosedur berkelanjutan?', html: `<button id="yesButton" class="btn btn-primary me-2">Ya</button><button id="noButton" class="btn btn-light">Tidak</button>`, showConfirmButton: false, didRender: () => { $('#yesButton').click(() => { Swal.close(); onYes(); }); $('#noButton').click(() => { Swal.close(); onNo(); }); } })
        };
        ...
        const FormSubmissionHandler = {
            submitFormData: (selections = {}) => {
                ...
                AlertHelper.confirmSave().then((result) => {
                    ...
                    if (Object.keys(selections).length > 0) Object.entries(selections).forEach(([key, value]) => { formData += `&${key}=${value || ''}`; });
                    
                    AjaxHelper.request({
                        method: form.getAttribute('method'), url: form.getAttribute('action'), data: formData, loadingMessage: 'Menyimpan Data ...',
                        success: (response) => {
                            UIHelper.setButtonState(submitButton, false);
                            if (response.status === 'failed') { toastr.error(response.message); return; }
                            if (response.error) { toastr.error(response.error); return; }
                            toastr.success(response.message);
                             // Redirect or reset? For now strict standard response handling
                        },
            ...
            handleRujukanFlow: (noSurat) => {
                const selections = { tujuanKunjungan: '', alasanTidakSelesai: '', prosedur: '', prosedurBerkelanjutan: '', prosedurTidakBerkelanjutan: '' };
                if (noSurat) {
                    SEPWizard.showTujuanKunjungan().then((result) => {
                        ...
                        if (result.value == 2) {
                            SEPWizard.showAlasanTidakSelesai().then((result2) => {
                                if (result2.isConfirmed) { selections.alasanTidakSelesai = result2.value; FormSubmissionHandler.submitFormData(selections); }
                            });
                        } else {
                            SEPWizard.showBerkelanjutanChoice(
                                () => { selections.prosedur = 1; SEPWizard.showProsedurBerkelanjutan().then((res) => { if(res.isConfirmed) { selections.prosedurBerkelanjutan = res.value; FormSubmissionHandler.submitFormData(selections); } }); },
                                () => { selections.prosedur = 0; SEPWizard.showProsedurTidakBerkelanjutan().then((res) => { if(res.isConfirmed) { selections.prosedurTidakBerkelanjutan = res.value; FormSubmissionHandler.submitFormData(selections); } }); }
                            );
                        }
                    });
                } else {
                    SEPWizard.showAlasanTidakSelesai().then((result) => { if (result.isConfirmed) { selections.tujuanKunjungan = 0; selections.alasanTidakSelesai = result.value; FormSubmissionHandler.submitFormData(selections); } });
                }
            }
        };
        ...
        const EventHandlers = {
                ...
                $('#btn-setuju-consent').click((e) => {
                    ...
                    validator.validate().then((status) => {
                        ...
                        AlertHelper.confirmSave('Simpan Consent').then((result) => {
                            if (!result.isConfirmed) return;
                             AjaxHelper.request({
                                method: 'POST', url: $('#form-consent').attr('action'), data: $('#form-consent').serialize(), showLoading: false,
                                beforeSend: () => UIHelper.showElementLoading('#kt_block_ui_4_target_consent'),
                                success: (response) => {
                                    UIHelper.hideElementLoading('#kt_block_ui_4_target_consent');
                                    if (response.status === 'success') { $('#btn-setuju-consent').addClass('d-none'); $('#general_concent').modal('hide'); $('#consent').html(AlertHelper.getConsentSuccessHTML()); }
                                    toastr.success(response.message);
                                },
                                error: () => UIHelper.hideElementLoading('#kt_block_ui_4_target_consent')
                            });
                        });
                    });
                });
            ...
            setupMainFormSubmit: (validator) => {
                $('#button-simpan').click((e) => {
                    e.preventDefault();
                    validator.validate().then((status) => {
                        if (status !== 'Valid') { toastr.warning('Periksa kembali kelengkapan data form.'); return; }
                        const noSurat = $('#no_surat').val();
                        const isRujukan = $('#is_rujukan').val(); // Logic note: #is_rujukan element might be dynamically added by SEP form
            ...
            setupSepManual: () => {
                $('#btn-sep-manual').click(() => {
                    $('#sep_rujukan').empty(); 
                    AjaxHelper.request({
                        url: '{{ route('buat-sep-manual') }}', data: { no_rm: '{{ $pasien->no_rm }}' },
                        success: (response) => {
                            if (response.status === 'failed') $('#insert_sep_manual').empty();
                            else {
                                $('#insert_sep_manual').html(response);
                                $('#btn-cari-dokter').prop('disabled', false);
                                $('#jenis_rawat').val(3); // Auto select IGD
            ...
            setupBuatSep: () => {
                $('#buat_sep').change(function() {
                    ...
                    if(checked) {
                        ...
                        $('#sep').val(1);
                        $('html, body').animate({ scrollTop: $("#card_buat_sep").offset().top - 100 }, 500);
                    } else {
            ...
            setupPoliChange: () => {
                $('#poli').change(function() {
                    ...
                    if (selectedValue > 0) {
                        if (poliRujukan != selectedValue) { $('.rujukan_kontrol').addClass('d-none'); $('#no_surat, #txtkddpjp, #txtnmdpjp').val(''); } 
                        else { $('.rujukan_kontrol').removeClass('d-none'); }
                        $('#btn-cari-dokter').prop('disabled', false);
                    } else {
            ...
            setupFaskesChange: () => {
                $('#faskes').change(function() {
                    ...
                    if (!faskes) { toastr.error('Pilih faskes'); $('#list_data_rujukan').empty(); return; }
                    AjaxHelper.request({
                        url: '{{ route('get-rujukan-faskes') }}', data: { faskes: faskes, no_bpjs: "{{ $pasien->no_bpjs }}" },
                        success: (response) => {
                            if (response.status === 'failed') { toastr.error(response.message); $('#list_data_rujukan').empty(); }
                            else { $('#list_data_rujukan').html(response.response); toastr.success(response.message); }
                        }
                    });
                });
            },
            setupCariDokter: () => {
                $('#btn-cari-dokter').click(function() {
                    ...
                    const poli = $('#poli').val();
                    const penanggung = $('#penanggung').val();
                    if (!tgl || !penanggung) { toastr.error('Lengkapi Tanggal dan Penanggung!'); return; }
                    
                    UIHelper.setButtonState(btn, true);
                    AjaxHelper.request({
                        url: '{{ route('get-jadwal-dokter') }}', data: { poli, tgl, penanggung }, showLoading: false,
                        success: (response) => {
        ...
        const PoliManager = {
            options: {
                '1': [ { value: '', text: 'Pilih Poli / Unit' }, { value: '2', text: 'BEDAH' }, { value: '3', text: 'GIGI' }, { value: '4', text: 'OBGYN' }, { value: '5', text: 'PENYAKIT DALAM' }, { value: '6', text: 'ANAK' }, { value: '7', text: 'Saraf' }, { value: '8', text: 'JIWA' }, { value: '9', text: 'PARU' }, { value: '10', text: 'ORTOPEDI' }, { value: '11', text: 'MATA' }, { value: '12', text: 'REHABMEDIK' }, { value: '13', text: 'GIGI PRSOSTHODONTI' }, { value: '14', text: 'KULIT' } ],
                '3': [ { value: '', text: 'Pilih Poli / Unit' }, { value: '1', text: 'UGD' } ]
            },
            updateOptions: () => {
                ...
                options.forEach(opt => { poliSelect.append($('<option>', { value: opt.value, text: opt.text })); });
            }
        };
        ...
        $(document).ready(function() {
            ...
            @if ($cek_finger_print['metaData']['code'] == 200) $('#alert-finger').removeClass('d-none'); @endif
        });
